// @ts-check
import { defineConfig } from 'astro/config';
import starlight from '@astrojs/starlight';
import catppuccin from "@catppuccin/starlight";

// https://astro.build/config
export default defineConfig({
	integrations: [
		starlight({
			title: 'Tensamin',
			plugins: [
				catppuccin({
					dark: { flavor: "mocha", accent: "lavender" },
					light: { flavor: "latte", accent: "lavender" },
				}),
			],
			social: [{ icon: 'github', label: 'GitHub', href: 'https://github.com/Tensamin' }],
			sidebar: [
				'index',
				'installation',
				{
					label: 'Components', items: [
						{ label: 'Iota', autogenerate: { directory: "components/iota" } },
						{ label: 'Omikron', autogenerate: { directory: "components/omikron" } },
						{ label: 'Omega', autogenerate: { directory: "components/omega" } },
						{ label: 'Frontend', autogenerate: { directory: "components/frontend" } },
						{ label: 'Auth Server', autogenerate: { directory: "components/auth-server" } },
					]
				},
				{ label: 'Security', autogenerate: { directory: 'security' } },
				{ label: 'Legal', autogenerate: { directory: 'legal' } },
			]
		}),
	],
});
